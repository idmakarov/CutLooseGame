    ...
    public class PhysicCable : MonoBehaviour
    {
        ...
        private void UpdatePoints()
        {
            ...
            for (int i = 0; i < length; i++)
                ...
                {
                    DestroyFine(transform.GetChild(i).gameObject);
                    length--;
                    i--;
                }
        ...
        public void AddPoint()
        {
            ...
            numberOfPoints++;
            if (Application.isPlaying) RebuildLists();
        }
        ...
        public void RemovePoint()
        {
            ...
            DestroyFine(lastprevPoint.gameObject);
            DestroyFine(lastprevCon.gameObject);
            // Also destroy the end connector that linked lastprevPoint -> end
            Transform endConn = GetConnector(numberOfPoints); // still old number here
            if (endConn != null) DestroyFine(endConn.gameObject);
        ...
        private void DestroyFine(GameObject obj)
        {
        ...
        private void DestroyFine(SpringJoint obj)
        {
            if (Application.isPlaying)
                Destroy(obj);
            else
        ...
        private void RebuildLists()
        {
            ...
            points.Add(start.transform);
            points.Add(point0.transform);   // ← was missing!
            connectors.Add(connector0.transform);
            ...
            if (points.Count != connectors.Count + 1)
                Debug.LogError($"List mismatch! points={points.Count}, connectors={connectors.Count}, numberOfPoints={numberOfPoints}");
        }

        [ContextMenu("Resize cable")]
        private void ResizeCable()
        {
            // Resize all points
            for (int i = 0; i < numberOfPoints; i++)
            {
            ...
            // Resize and reposition all connectors
            Transform lastPoint = start.transform;
        
            for (int i = 0; i < numberOfPoints; i++)
            {
                Transform point = GetPoint(i);
                Transform connector = GetConnector(i);
        
                if (point != null && connector != null)
                {
                    connector.position = CountConPos(lastPoint.position, point.position);
                    connector.rotation = CountRoationOfCon(lastPoint.position, point.position);
                    connector.localScale = CountSizeOfCon(lastPoint.position, point.position);
                }
            ...
            // Last connector (last point -> end)
            Transform endConnector = GetConnector(numberOfPoints);
            if (endConnector != null)
            {
                endConnector.position = CountConPos(lastPoint.position, end.transform.position);
                endConnector.rotation = CountRoationOfCon(lastPoint.position, end.transform.position);
                endConnector.localScale = CountSizeOfCon(lastPoint.position, end.transform.position);
            }
        }
